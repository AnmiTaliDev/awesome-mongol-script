# Awesome Mongol Script [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

<div align="center">

### ᠮᠡᠨᠳᠦ ᠪᠠᠶᠢᠨ᠎ᠠ᠂ ᠬᠦᠮᠦᠨ ᠡᠴᠢ ᠳᠡᠭᠦᠦ ᠬᠥᠮᠦᠰ
(Mendu baina, hümün echi degüü hömüs / Greetings, dear human siblings!)

</div>

> A curated list of awesome open-source projects, libraries, tools, and resources for Traditional Mongolian Script (ᠮᠣᠩᠭᠣᠯ ᠪᠢᠴᠢᠭ᠌).

**Read this in other languages:** [English](README.md) | [Монгол (Кирилл)](README_mn.md) | [Қазақша](README_kk.md) | [Русский](README_ru.md) | [ᠮᠣᠩᠭᠣᠯ ᠪᠢᠴᠢᠭ᠌](README_mn_script.md)

## What is Traditional Mongolian Script?

Traditional Mongolian Script (ᠮᠣᠩᠭᠣᠯ ᠪᠢᠴᠢᠭ᠌, Mongol bichig) is the classical and traditional writing system of the Mongolic languages. It has a history spanning over 800 years and is one of the few vertical scripts still in use today. The script is written vertically from top to bottom in columns that advance from left to right.

Today, Traditional Mongolian Script is:
- The official script of Inner Mongolia (China)
- Experiencing a revival in Mongolia, where it's being reintroduced alongside Cyrillic
- Used by Mongolian communities worldwide to preserve cultural heritage
- Supported in Unicode (U+1800–U+18AF) and modern digital systems

## Contents

- [Converters](#converters)
- [Fonts](#fonts)
- [Input Methods](#input-methods)
- [Libraries](#libraries)
- [Text Editors](#text-editors)
- [Educational Resources](#educational-resources)
- [Mobile Apps](#mobile-apps)
- [Web Tools](#web-tools)
- [Research & Standards](#research--standards)
- [Contributing](#contributing)

## Converters

Tools for converting between different scripts and encodings.

- [mongol_converter](https://github.com/suragch/mongol_converter) - A Cyrillic to Mongolian converter built with Flutter. The backend is hosted with PocketBase.

## Fonts

Open-source fonts supporting Traditional Mongolian Script.

- [Noto Mongolian](https://github.com/notofonts/mongolian) - Google's Noto font family for Mongolian script, part of the Noto Fonts project.

## Input Methods

*No entries yet. See [contribution guidelines](CONTRIBUTING.md) to add input methods.*

## Libraries

Development libraries and packages for working with Mongolian Script.

- [mongol-library](https://github.com/suragch/mongol-library) - Android UI components for vertical Mongolian text.
- [mongol](https://github.com/suragch/mongol) - Collection of Flutter widgets for displaying traditional Mongolian vertical text.

## Text Editors

*No entries yet. See [contribution guidelines](CONTRIBUTING.md) to add text editors.*

## Educational Resources

*No entries yet. See [contribution guidelines](CONTRIBUTING.md) to add educational resources.*

## Mobile Apps

*No entries yet. See [contribution guidelines](CONTRIBUTING.md) to add mobile apps.*

## Web Tools

*No entries yet. See [contribution guidelines](CONTRIBUTING.md) to add web tools.*

## Research & Standards

Standards, specifications, and academic research related to Mongolian Script.

*No entries yet. See [contribution guidelines](CONTRIBUTING.md) to add research papers or standards.*

## Contributing

Contributions are welcome! Please read the [contribution guidelines](CONTRIBUTING.md) first.

## License

[![CC0](https://licensebuttons.net/p/zero/1.0/88x31.png)](https://creativecommons.org/publicdomain/zero/1.0/)

To the extent possible under law, [AnmiTaliDev](https://github.com/AnmiTaliDev) has waived all copyright and related or neighboring rights to this work.